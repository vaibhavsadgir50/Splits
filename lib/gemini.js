...
// Judges scraped image-search candidates the way a person would — reading
// what each result actually is, not just counting overlapping words. Used
// for DuckDuckGo/Bing specifically, since those are the two sources that
// have produced confidently-wrong matches (e.g. a "Zara" brand of paste
// matching the Zara clothing retailer on a bare word-overlap check).
// Returns the matching candidate's index, or -1 if Gemini determined none
// genuinely show the item (never guesses just to fill in an answer) — vs.
// throwing if the call itself failed, so callers can tell "confirmed no
// match" apart from "couldn't ask" and fall back accordingly.
export async function pickRelevantImageMatch(itemName, candidates) {
  if (!candidates.length) return -1

  const listText = candidates
    .map((c, i) => `[${i}] title: "${c.title || ''}"${c.desc ? `\n    desc: "${c.desc}"` : ''}`)
    .join('\n')
    ...
    contents: [
      {
        ...
        parts: [
          {
            text:
              `An item from a grocery/shopping receipt: "${itemName}"\n\n` +
              `Candidate image search results:\n${listText}\n\n` +
              'Which candidate, if any, is genuinely a photo of this specific item? Be careful with ' +
              'brand names that happen to look like something else entirely (e.g. a paste brand called ' +
              '"Zara" is not the Zara clothing retailer; a snack brand called "Total" is not the word "total"). ' +
              'Return a raw JSON object only, no markdown: {"index": N} for the matching candidate, or ' +
              '{"index": -1} if none of them actually show this item — do not guess just to give an answer.',
          },
        ],
      },
    ],
  ...
  return Number.isInteger(index) && (index === -1 || candidates[index]) ? index : -1
}
