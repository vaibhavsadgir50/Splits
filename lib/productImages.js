...
async function searchDuckDuckGo(query) {
  try {
    ...
    console.log(`[productImages] duckduckgo: ${results.length} results for "${query}" but none matched the query`)
    return null
  } catch (err) {
...
async function searchBingImages(query) {
  try {
    ...
    if (!matches.length) { console.log(`[productImages] bing: no results for "${query}" (likely blocked)`); return null }
    ...
    console.log(`[productImages] bing: ${matches.length} results for "${query}" but none matched the query`)
    return null
  } catch (err) {
